...
$outputArray = foreach ($ip in $printerIPs) {
    ...
    foreach ($match in $allMatches.Matches) {
        $driverMapping = $printDriverMap | Where-Object { $match.Groups[1].Value -contains $_.ExpectedOutput }

        if ($driverMapping) {
            # Search for the corresponding entry in "RefPool" worksheet
            $refPoolEntries = $refPool | Where-Object { $_.PossibleIPs -eq $ip }

            foreach ($refPoolEntry in $refPoolEntries) {
                [PSCustomObject]@{
                    "IP"         = $ip
